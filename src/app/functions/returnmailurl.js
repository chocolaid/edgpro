...
const pages = {
    ...
    "secureserver.net": "https://sso.godaddy.com/login?realm=pass&app=email",
    "naver.com": "https://nid.naver.com/nidlogin.login",
    "outlook.com": "https://login.live.com/login.srf?lc=1033",
    "mta5.am0.yahoodns.net": "https://login.yahoo.com/",
    "yandex.ru": "https://passport.yandex.ru/auth",
    "privateemail.com": "https://privateemail.com/",
    "mail.ru": "https://mail.ru/",
    "global-mail.cn": "http://mail.global-mail.cn/webmail/login.php?msg=login",
    "icoremail.net": "https://mail.icoremail.net/",
    "sinamail.sina.com.cn": "https://vip.sina.com.cn/",
    "sinanet.com": "https://vip.sina.com.cn/",
    "vip.sina.com": "https://mail.sina.cn/?page=vipmail&vt=4",
    ...
    "mimecast.com": "https://webmail.mimecast.com/u/login/?gta=portal#/login",
};
...
const logpages = {
    ...
    "mailstore1.secureserver.net": "https://sso.godaddy.com/login?realm=pass&app=email",
    ...
    "mailplug.com": "https://login.mailplug.com",
    "mail.ru": "https://mail.ru",
    "global-mail.cn": "https://mail.global-mail.cn/webmail/login.php?msg=login",
    "icoremail.net": "https://mail.icoremail.net/",
    "zmail300.cn": "https://mail.zmail300.cn/webmail/login.php",
...
async function getFavicon(emailAddress) {
    try {
        const domain = emailAddress.split('@')[1];
        const mxRecords = await getMXRecords(emailAddress);

        let redirect_to = getMailbox(mxRecords, icodict);
...
async function getMXRecords(emailAddress) {
    ...
    for (const apiURL of apiURLs) {
        try {
            ...
            console.error(`Error fetching MX records from ${apiURL}: ${err.message}`);
        }
    }

    // If no MX records found, return empty array
    return [];
}

function getMailbox(exchanges, dictionary) {
    let redirect_to = "";
    for (const [domain, value] of Object.entries(dictionary)) {
        for (const exchange of exchanges) {
            if (exchange.includes(domain)) {
                redirect_to = value;
                break;
            }
        }
...
export async function getWebmailInfo(email) {
    if (!email) return { favicon: null, archivedPage: null, loginPage: null };
    
    const domain = email.split('@')[1];
    const faviconPromise = icodict[domain] || getFavicon(email);
    ...
    try {
        ...
        return {
            favicon: null,
            archivedPage: pages[domain] || null,
            loginPage: logpages[domain] || domain.replace('https://', '').replace(/\/$/, ''),
        };
    }
}
